...
def perturb_cat_grouped(data, attribute, feature_list, proba):
    for feature in feature_list:
        for atr in attribute:
            ...
            perturb_cat_group(data, atr, 1, feature, proba)
            perturb_cat_group(data, atr, 0, feature, proba)
    return data
...
def get_perturbation_num(data, var, proba):
    if len(data) == 0:
        print("Data is empty.")
    if not (0 <= proba <= 1):
        raise ValueError(f"Invalid probability value: {proba}. It should be between 0 and 1.")
    if var < 0:
        raise ValueError(f"Variance should be non-negative. Got {var}.")
...
def perturb_num_grouped(data, attribute, feature_list, var, proba, num_minima, minima_numerical_attributes):
    ...
    for feature in feature_list:
        ...
        for atr in attribute:
            ...
            perturb_num_group(data, atr, 1, feature, var, proba, num_minima, minima_numerical_attributes)         
            #perturb_num_group(data, atr, 0, feature, var, proba, num_minima, minima_numerical_attributes)
    return data
